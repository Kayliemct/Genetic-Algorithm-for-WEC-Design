...
%
function [Mass,Inertia,KH,XB,YB,ZB]=axiMeshGA2(r,z,n,zCG,p)
rho=1000;               %fresh water or seawater
g=9.81;
ntheta=50;              %50 sections
name=sprintf('%d',p);   %Individuals in a population are saved in separate folders, repeated each gen
nPanel=500;             %500 target panels

% status=close('all');   %commented this out, may need it
...
system(['mkdir ','C:\Users\labuser\Documents\Kaylie\GA\100GenParallel\',nomrep,'\',nomrep]);   %change all to \nomrep\nomrep
system(['mkdir ','C:\Users\labuser\Documents\Kaylie\GA\100GenParallel\',nomrep,'\',nomrep,filesep,'mesh']);     %check
system(['mkdir ','C:\Users\labuser\Documents\Kaylie\GA\100GenParallel\',nomrep,'\',nomrep,filesep,'results']);  %check
% Creation des fichiers de calcul du maillage
fid=fopen(fullfile('C:\Users\labuser\Documents\Kaylie\GA\100GenParallel\',nomrep,'Mesh.cal'),'w');          %check
fprintf(fid,'axisym \n',1);
fprintf(fid,'1 \n 0. 0. \n ');
zG=zCG;  %input(' - Vertical position of gravity center : ');
fprintf(fid,'%f %f %f \n',[0. 0. zG]);
nfobj=nPanel;  %input(' - Target for number of panels : ');
fprintf(fid,'%g \n 2 \n 0. \n 1.\n',nfobj);
fprintf(fid,'%f \n %f \n',[rho g]);
status=fclose(fid);
fid=fopen(fullfile('C:\Users\labuser\Documents\Kaylie\GA\100GenParallel\',nomrep,'ID.dat'),'w');            %check
fprintf(fid,['% g \n',nomrep,' \n'],length(nomrep));
status=fclose(fid);
fid=fopen(fullfile('C:\Users\labuser\Documents\Kaylie\GA\100GenParallel\',nomrep,nomrep,'\mesh','axisym'),'w');    %,[nomrep,filesep,'mesh',filesep,'axisym']),'w')%;    %check
fprintf(fid,'%g \n',nx);
fprintf(fid,'%g \n',nf);
for i=1:nx
    fprintf(fid,'%E %E %E \n',[x(i) y(i) z(i)]);
...
if l == 1
    ...
    system('.\Mesh\Mesh.exe >Mesh\Mesh.log');       %changing directory per individual
end
% Visualisation du maillage
clear x y z NN nx nf nftri tri u v w;
fid=fopen(fullfile('C:\Users\labuser\Documents\Kaylie\GA\100GenParallel\',nomrep,nomrep,'\mesh','axisym.tec'),'r');     %,[nomrep,filesep,'mesh',filesep,'axisym.tec']),'r');    %check
...
clear KH;
KH=zeros(6,6);
fid=fopen(fullfile('C:\Users\labuser\Documents\Kaylie\GA\100GenParallel\',nomrep,nomrep,'\mesh','KH.dat'),'r');%,[nomrep,filesep,'mesh',filesep,'KH.dat']),'r');
for i=1:6   
    ligne=fscanf(fid,'%g %g',6);
    KH(i,:)=ligne;
end;
...
Inertia=zeros(6,6);
fid=fopen(fullfile('C:\Users\labuser\Documents\Kaylie\GA\100GenParallel\',nomrep,nomrep,'\mesh','Hydrostatics.dat'),'r');%,[nomrep,filesep,'mesh',filesep,'Hydrostatics.dat']),'r');
ligne=fscanf(fid,'%s',2);
XB=fscanf(fid,'%f',1);
ligne=fgetl(fid);
ligne=fscanf(fid,'%s',2);
...
status=fclose(fid);
clear ligne
fid=fopen(fullfile('C:\Users\labuser\Documents\Kaylie\GA\100GenParallel\',nomrep,nomrep,'\mesh','Inertia_hull.dat'),'r');%,[nomrep,filesep,'mesh',filesep,'Inertia_hull.dat']),'r');
for i=1:3
...
fid=fopen(fullfile('C:\Users\labuser\Documents\Kaylie\GA\100GenParallel\',nomrep,nomrep,'Nemoh.cal'),'w');%,[nomrep,filesep,'Nemoh.cal']),'w');
